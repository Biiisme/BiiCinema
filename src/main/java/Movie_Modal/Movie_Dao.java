...
public class Movie_Dao {
	...
	public Boolean updateMovie(Movie mv) throws SQLException, ClassNotFoundException{
		...
		KetNoi kn = new KetNoi();
		kn.ketnoi();
		String sql = "Update Movie set TenPhim=?, poster_url=?, MoTaNgan=?, MoTaDai=?, DaoDien=?, DienVien=?, trailer_url=?, Loai=?, NgayKhoiChieu=?, DoiTuong=?, ThoiLuong=? where movie_id=?";
		PreparedStatement cmd = kn.cn.prepareStatement(sql);
	...
	public Movie ChiTietphim(int movie_id) throws ClassNotFoundException, SQLException{
		...
		PreparedStatement cmd = kn.cn.prepareStatement(sql);
		cmd.setInt(1, movie_id);
		ResultSet rs = cmd.executeQuery();
		Movie movie = null;
		if(rs.next()) {
			movie = new Movie(rs.getInt("movie_id"),rs.getString("TenPhim"),rs.getString("poster_url"), 
					rs.getString("MoTaNgan"), rs.getString("MoTaDai"), rs.getString("DaoDien"), rs.getString("DienVien"),
					rs.getString("trailer_url") , rs.getString("Loai"),rs.getString("DoiTuong"),rs.getString("NgayKhoiChieu"),rs.getInt("ThoiLuong"));
		}
		rs.close();
